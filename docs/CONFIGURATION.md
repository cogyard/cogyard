...
| `COGYARD_INTEGRATION` | auto-detect → no-op | Which agent adapter is active (e.g. `claude`). Falls back to `~/.cogyard/config.json`, then auto-detection, then a built-in no-op (runs fine with no agent). See [INTEGRATIONS.md](INTEGRATIONS.md). |
...
(read by the worktree hooks and integration auto-detection), and `COGYARD_DEV_GUARD`
...
```jsonc
{
  "integration": "claude",        // active agent adapter
  ...
  "defaults": {                   // New / Add-existing drawer prefill
    ...
    "store": "shared"             // shared | normal
  }
}
```

- `integration` — the active agent adapter, same values as `COGYARD_INTEGRATION`
...
- `defaults` — the project-creation defaults the portal's New / Add-existing drawer
  prefills with (and the `/settings` view edits). Invalid `kind`/`store` values are
  ignored, falling back to the built-ins (`single` / `shared`). There is no default
  git `remote` — it's inherently per-project, so the drawer's remote field is always
  entered fresh.
...
and `integration` are shown read-only there — `COGYARD_HOME` is where `config.json`
itself lives (env/CLI-only), and the integration selector arrives with a second
adapter.

**`PROJECTS_ROOT` resolution** (task 060): `COGYARD_PROJECTS_ROOT` env →
`config.json.projectsRoot` → `~/gitroot`, mirroring how `integration` resolves. It's
...
| `_tasks/` | tracked dir **or** gitignored symlink | `init` / `convert` | The task store — a normal directory (`--store normal`) or a symlink into the shared store (`--store shared`, the default). |
