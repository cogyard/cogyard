...
- **Never hand-edit `claimed_at` / `claimed_by` / `claimed_by_session`.** Use
  `cogyard env claim` / `cogyard env release` — they're atomic, and on team
  stores they also pull/push the claim (see `docs/MULTI-USER.md`). *(hook-enforced)*
