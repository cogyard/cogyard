...
name: pickup-task
description: |
  Pick up an existing `_tasks/NNN-*.md` file and work it. Trigger when the user says "do task NNN", "pick up task X", "work on task Y", "show me task Z", "continue task NNN", "resume task X", or otherwise references an existing task file by id — AND on any continuation-shaped request: naming a task's worktree or branch as the place to work ("get back in the worktree", "fix it in that worktree"), or asking for follow-up work on a task this session already worked ("fix what broke", "one more change"), even after that task merged or was marked DONE. The word "worktree" alone is never a cue for a bare EnterWorktree call — if the destination is a task's recorded worktree, this skill's continuation flow wraps it. The skill handles the full pickup ritual: sync, env detection, claim check, worktree (CONTINUATION — reuse the task's existing worktree+branch if one is recorded, asking before rebasing onto main; merged/DONE tasks rehydrate the same way; otherwise create a task-named worktree, or say "no worktree" / "on main" to work directly on main), drift check, claim, work, release.

  Different from `write-task`: write-task creates a NEW task file from a description. pickup-task reads, claims, and executes an EXISTING task. They share the cogyard storage model (per-project `_tasks/`, normal dir or shared-store symlink) — see the "The `_tasks/` storage model" section in the `write-task` skill.
---
...
- Any reference to a task's WORKTREE or BRANCH as the place to work — "get back in the worktree", "fix it in that worktree", "on the task-NNN branch". Resolve which task's recorded `env.worktree`/`env.branch` matches; that task is being resumed.
- Any follow-up work request on a task this session already worked — "fix what broke", "one more change to that", "adjust it" — even without the word "task", and **even after the task merged, was released, or was marked DONE**. Follow-up work on a known task is always a continuation pickup, never ad-hoc.

**Classification rule — skill before tool.** The word "worktree" in a request is NOT a cue to call EnterWorktree directly. If the destination is an existing task's recorded worktree, the request is a continuation trigger for THIS skill (whose Step 5 wraps EnterWorktree in the full ritual). EnterWorktree is called bare only when the destination is not tied to any task.

Do NOT use for:
- Authoring a new task — use `write-task` instead.
- Browsing the task list — point user at `/cogyard` slash command.
- Quick in-session todos — use the session task list (TaskCreate/TaskUpdate, or TodoWrite in older Claude Code versions), not the file system.
...
If `frontmatter.env.claimed_at` is set and `frontmatter.env.claimed_by_session` is NOT this session, STOP. Tell the user (prefer the human holder from `env.claimed_by` when present — on team stores it may be a teammate, not another of the user's own sessions):

> "Task NNN is claimed by <claimed_by, else session X> on planet Y, worktree Z since DATE. Continue (re-claim — will displace the other session), pick a different task, or coordinate with the other session?"

Wait for direction. Don't claim over another session without confirmation — and be extra reluctant when `claimed_by` names someone other than the current user.

**Expected case, not an anomaly:** claims persist until merge (Step 11), so a CONTINUATION of the user's own task normally finds the claim held by a dead prior session of the same user (`claimed_by` = this user, `env.worktree` matches the recorded worktree). Confirm briefly and re-claim with `--force` — that's the designed hand-over, not a collision.
...
**Merged / DONE tasks reopen the same way.** A task whose branch already merged (status DONE, claim released) that gets follow-up work — a bug in what it shipped, a tweak, a revert — is STILL a continuation, not a fresh start and not direct-on-main work. Rehydrate: reuse the recorded worktree+branch (steps 1–3 below), ask the rebase question (step 4 — after a merge, rebasing onto the base branch typically fast-forwards the branch to current base), re-claim, then work. The DONE status stays until the user re-opens it or signs off the fix; the claim is what marks the worktree active again.
...
This atomically sets `claimed_at` (now, ISO timestamp), `claimed_by` (the human holder: `$COGYARD_USER`, else `git config user.name`, else `$USER`), and `claimed_by_session`. Refuses with exit code 2 if already claimed by another session — handle that case per Step 4; the refusal JSON names the holder in `claimed_by`.

On a remote-backed team store (origin + `tasks` branch), `claim` pulls first and pushes the claim commit itself — teammates see it immediately, and a lost race comes back as the normal already-claimed refusal, never a rebase conflict. See `docs/MULTI-USER.md`.

Then push the box-tick/edit state so other sessions see it (on team stores the claim itself is already pushed; this publishes any accompanying task-file edits):
...
### 11. Claim lifecycle — claims persist until MERGE

A claim means "owned — not available." An unmerged task is not done, so
**finishing the work does NOT release the claim.** Ending a session with
acceptance met but the branch unmerged: KEEP the claim — the task shows as
claimed-by-you (in review), which is the truth. Only two events release:

- **Merge.** `/merge-to-main` closes the task (status DONE + `done_date`) and
  releases the claim as its own step — nothing to do at session end.
- **Returning the task to the pool** (switching away, wrong pickup, work
  discarded — the task should be available to others again):

```
cogyard env release <task-file-path>
cogyard tasks sync push "release NNN — back in the pool"
```

If switching tasks, re-run the Pickup Procedure for the new task starting at Step 1. A later continuation session re-claims over your own stale session id via the Step 4 flow.
...
Keep working through the task's full scope unless you hit a real boundary:
...
- **(b) File it as a new task** via the `write-task` skill (or the host app's spawn-task feature, if one is available in this session). Use when the new work is large enough to deserve its own pickup brief.
...
- **(b) Spin remainder into a new `_tasks/NNN-*.md`** by writing a fresh task file via `write-task` (or the host app's spawn-task feature, if available).
...
Always offer; do not silently leave it ambiguous. Then apply Step 11's claim lifecycle: work awaiting merge KEEPS the claim (in review); release only if the task goes back to the pool.
...
- **Claim state that lies about ownership.** Two failure modes: releasing a finished-but-UNMERGED task (it looks available while it's actually in review — the gap this rule closes), and leaving an ABANDONED task claimed (blocks the next session for nothing). The invariant: merged → closed+released by `/merge-to-main`; awaiting merge → claimed; back in the pool → released.
- **Bare EnterWorktree into a task's recorded worktree.** "Get back in the worktree" is a continuation trigger for this skill, not a navigation command — entering ad-hoc skips the staleness gate, the rebase question, and the claim. If the target worktree matches any task's `env.worktree`, run the Step 5 continuation flow instead.
- **Treating a merged/DONE task's follow-up as ad-hoc work.** Fixes to something a task shipped go through the same continuation ritual in that task's worktree — never directly on the base branch, never in an unclaimed checkout.

## See also

- The user's `~/.claude/CLAUDE.md` (auto-loaded) — per-user task-system conventions and hard rules typically live there; honor them when present.
- `write-task` skill, "The `_tasks/` storage model" section — the storage model reference (ships with this plugin).
...
- `/cogyard` — slash command that boots the dashboard (ships with this plugin).
