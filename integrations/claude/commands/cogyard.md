...
WHAT YOU ACTUALLY DO
  ...
  See status       Go to the portal: http://cogyard  (live Angular UI — the only dashboard).
...
If the page fails to load, the LaunchAgent may be unloaded. Re-bootstrap with:

```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cogyard.serve.plist
```

(To stop it, rare: `launchctl bootout gui/$(id -u)/com.cogyard.serve`.)
