...
- **Continuation** — same task, next session. Heading: `Continuing <task> at session N`. Cite work done so far (commits, current TodoWrite state).
...
- `ls <project>/_docs/` — note which docs exist; the brief will reference them by name.
- `ls <project>/_tasks/` — note recent task docs.
- `find ~/.claude/projects/$(echo "$PWD" | tr / -)/memory/ -type f -name '*.md'` — list project memory files. The brief should point at them by name.
...
- Active TodoWrite list (from this session) — to reflect in-flight items.
...
Pull from:
- `<project>/CLAUDE.md` — the "Critical rules" + "What not to do" lists.
- The memory file capturing the project's architecture rules (if any — e.g. `project_architecture_rules.md`).
- This user's cross-project rules (no compound shell commands; this lives in `feedback_no_compound_shell.md` in memory).
...
```
<Continuing|Starting> <task title> at <project name> (~/gitroot/<project>).
...
2. _docs/IMPLEMENTATION.md — <which session / which row in the status table>
3. _docs/PROJECT.md — <which section>
4. _docs/DECISIONS.md — <recent entries that bear on this work>
5. _tasks/<NNN>-<slug>.md — <the task being executed>
6. ~/.claude/projects/<sanitized-project-path>/memory/<files> — <which memory>

LIVE STATE (don't recreate):
- Deployed: <url> — currently on commit <sha> per /health.
- Working API key / admin password: see memory file `project_test_state.md`.
...
- Update _docs/IMPLEMENTATION.md status table.
- Update _docs/DECISIONS.md if anything material was decided.
...
- **Reference, don't inline.** Point at files (`memory/<name>`, `_docs/<name>`) for content that already lives somewhere durable. Inlining duplicates content that will drift.
...
A backend-cleanup ran across two days. Day 1 completed Tier A; Day 2 needs to do Tier B. Filename: `_tasks/005-handoff-backend-cleanup.md`. Heading: "Continuing the 005 backend cleanup at session 2." Live state: which Tier A items committed (with SHAs), what's in `git status`, which TodoWrite items are pending.
