// routes/activity.mjs — activity-view endpoints (task 064). Read-only GETs,
...
export async function handle(path, u, projects, res) {
  if (path === '/api/activity') {
    ...
    const commits = {};
    await Promise.all(projects.map(async (p) => { commits[p.slug] = await core.commitsPerDay(p, days); }));
    return json(res, 200, { days, commits, punchcards: core.activityPunchcard(), ...core.activityRollup(days) });
  }
