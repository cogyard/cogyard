// test/integrations.test.mjs — the agent-integration seam (task 038).
...
import { NOOP, resolveActive, loadAdapter, listIntegrationNames } from '../core/integrations.mjs';
...
test('resolveActive honors COGYARD_INTEGRATION=none → no-op', async () => {
  const prev = process.env.COGYARD_INTEGRATION;
  process.env.COGYARD_INTEGRATION = 'none';
  try { assert.equal((await resolveActive()).name, 'none'); }
  finally { if (prev === undefined) delete process.env.COGYARD_INTEGRATION; else process.env.COGYARD_INTEGRATION = prev; }
});

test('resolveActive degrades to no-op for an unknown integration name', async () => {
  const prev = process.env.COGYARD_INTEGRATION;
  process.env.COGYARD_INTEGRATION = 'does-not-exist';
  try { assert.equal((await resolveActive()).name, 'none'); }
  finally { if (prev === undefined) delete process.env.COGYARD_INTEGRATION; else process.env.COGYARD_INTEGRATION = prev; }
});

test('the claude integration is discoverable on disk', () => {
  assert.ok(listIntegrationNames().includes('claude'));
});
